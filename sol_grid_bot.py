"""
BCVertex — Grid Bot SOL/USDT
SOL tem volatilidade ~40% maior que ETH → range 14% para capturar oscilações.
Activo apenas em fase bull/lateral — em bear, CFO mantém DCA.
Capital: configurável via SOL_GRID_CAPITAL (default $100 USDT).
"""
...
CONFIG = {
    "symbol":        "SOL/USDT",
    "capital":       float(os.getenv("SOL_GRID_CAPITAL", "100")),
    "n_levels":      10,
    "range_pct":     0.14,   # ±7% — SOL mais volátil que ETH
    "capital_pct":   0.90,
    "max_open":      5,
    "fee":           0.001,
}

STATE_FILE = Path("memory/sol_grid_state.json")
...
def _inicializar_grid(price: float, state: dict) -> dict:
    ...
    state["ref_price"] = round(price, 4)
    state["level_size"] = round(level_size, 4)
    state["capital_per_level"] = round(capital_por_nivel, 4)
    state["created_at"] = datetime.now(timezone.utc).isoformat()
    state["open_positions"] = {}
    logger.info(
        f"[SOL Grid] Iniciado @ ${price:.4f} | "
        f"Range ${price - CONFIG['n_levels']//2 * level_size:.4f}–"
        f"${price + CONFIG['n_levels']//2 * level_size:.4f} | "
        f"Nível: ${level_size:.4f} | Capital/nível: ${capital_por_nivel:.4f}"
    )
    return state
...
def run_cycle() -> dict:
    ...
    if not state.get("active", True):
        return {"status": "pausado", "message": "SOL Grid Bot pausado."}
    ...
    try:
        ...
        for nivel_str, pos in list(open_positions.items()):
            ...
            if price >= target_exit:
                ...
                acoes.append(f"VENDA nivel {nivel_str} @ ${price:.4f} | PnL ${pnl_net:+.4f}")
                logger.info(f"[SOL Grid] SELL nivel {nivel_str} @ ${price:.4f} | PnL ${pnl_net:+.4f}")
        ...
        if niveis_para_entrar and len(open_positions) < CONFIG["max_open"]:
            ...
            if nivel_entrada < nivel_actual:
                size = round(capital_per_level / price, 4)
                min_notional = 10.0
                if size * price >= min_notional:
                    ...
                    acoes.append(f"COMPRA nivel {nivel_entrada} @ ${price:.4f} | size {size}")
                    logger.info(f"[SOL Grid] BUY nivel {nivel_entrada} @ ${price:.4f} | size {size}")
        ...
        return {
            ...
            "acoes": acoes,
            "grid_range": {
                "lower": round(ref + min_nivel * level_size, 4),
                "upper": round(ref + (CONFIG["n_levels"] // 2) * level_size, 4),
            },
        }

    except Exception as e:
        logger.error(f"[SOL Grid] Erro: {e}")
...
def get_status() -> dict:
    ...
    if ref and lvl:
        grid_range = {
            "lower": round(ref - 5 * lvl, 4),
            "upper": round(ref + 5 * lvl, 4),
        }
    return {
        "bot": "Grid SOL/USDT",
...
def reset_grid():
    ...
    state["created_at"] = ""
    save_state(state)
    logger.info("[SOL Grid] Grid resetado.")
