"""
BCVertex — ETH/USDT Bot
Estratégia configurável pelo CFO após aprovação do Vasco:
  - "grid"          : 10 níveis ±5% — mercado lateral (ETH mais volátil que BTC)
...
def _run_dca(state: dict, ex, price: float) -> dict:
    ...
    if state.get("dca_qty", 0.0) > 0 and price >= ref * (1 + cfg["sell_pct"]):
        ...
        state["dca_qty"] = 0.0
        state["dca_ref"] = price
        state["trades"].append({"tipo": "dca_venda", "price": round(price, 4), "size": round(size, 6), "pnl": round(pnl, 4), "ts": datetime.now(timezone.utc).isoformat()})
        if not TESTNET:
            ex.create_order(SYMBOL, "market", "sell", size)
        acoes.append(f"DCA VENDA {size:.6f} ETH @ ${price:.4f} | PnL ${pnl:+.4f}")
        ...
        if size * price >= 10:
            ...
            state["dca_ref"] = price
            state["trades"].append({"tipo": "dca_compra", "price": round(price, 4), "size": size, "ts": datetime.now(timezone.utc).isoformat()})
            if not TESTNET:
                ex.create_order(SYMBOL, "market", "buy", size)
            acoes.append(f"DCA COMPRA {size:.6f} ETH @ ${price:.4f}")
    ...
    if state.get("ts_qty", 0.0) == 0.0:
        ...
        if cash > 10:
            ...
            if size * price >= 10:
                ...
                if not TESTNET:
                    ex.create_order(SYMBOL, "market", "buy", size)
                acoes.append(f"TS COMPRA {size:.6f} ETH @ ${price:.4f}")
                logger.info(f"[ETH TS] COMPRA {size:.6f} @ ${price:.4f}")
    else:
        ...
        if price <= stop_price:
            ...
            if not TESTNET:
                ex.create_order(SYMBOL, "market", "sell", size)
            acoes.append(f"TS VENDA {size:.6f} ETH @ ${price:.4f} | PnL ${pnl:+.4f}")
            logger.info(f"[ETH TS] VENDA {size:.6f} @ ${price:.4f} | PnL ${pnl:+.4f}")
...
def _run_grid_cycle(state: dict, ex, price: float) -> dict:
    ...
    for nivel_str, pos in list(open_positions.items()):
        ...
        if price >= entry + level_size:
            ...
            state["trades"].append({"nivel": int(nivel_str), "entry": entry, "exit": round(price, 4), "size": size, "pnl": round(pnl_net, 4), "closed_at": datetime.now(timezone.utc).isoformat()})
    ...
    niveis_para_entrar = [n for n in range(min_nivel, nivel_actual) if str(n) not in open_positions and n < nivel_actual]
    if niveis_para_entrar and len(open_positions) < CONFIG["max_open"]:
        ...
        if nivel_entrada < nivel_actual:
            ...
            if size * price >= 10:
                if not TESTNET:
                    ex.create_order(SYMBOL, "market", "buy", size)
                open_positions[str(nivel_entrada)] = {"entry": round(price, 4), "size": size, "opened_at": datetime.now(timezone.utc).isoformat()}
...
def run_cycle() -> dict:
    ...
    if not state.get("active", True):
        return {"status": "pausado", "message": "ETH Bot pausado."}
...
def set_estrategia(nova: str) -> dict:
    ...
    if nova == "grid":
        ...
        state["dca_qty"] = 0.0
    save_state(state)
    logger.info(f"[ETH Bot] Estratégia: {anterior} → {nova}")
...
def get_status() -> dict:
    ...
    return {
        ...
        "ts_peak": ts_peak,
        "ts_stop": round(ts_peak * (1 - TRAILING_CONFIG["trail_pct"]), 4) if ts_peak else None,
    }
