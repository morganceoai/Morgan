...
THEMES = {
    "indigo": {
        "P":  colors.HexColor("#1a4f7a"),  # primary (navy)
        "Pd": colors.HexColor("#0f3557"),  # primary dark
        "Pl": colors.HexColor("#e8f2f9"),  # primary light
        "A":  colors.HexColor("#0099cc"),  # accent (ocean blue)
        "R":  colors.HexColor("#c0d8e8"),  # rule lines
        "BG": colors.HexColor("#f4f9fc"),  # page bg
        "TX": colors.HexColor("#0d2035"),  # text
        "MU": colors.HexColor("#6a98b8"),  # muted
        "WE": colors.HexColor("#dff0f7"),  # weekend bg
        "WA": colors.HexColor("#0099cc"),  # weekend accent
    },
    "sage": {
...
DAYS = [("Mo","Montag"),("Di","Dienstag"),("Mi","Mittwoch"),
...
def cover(cv, t, tname):
    ...
    cv.setFillColor(colors.white)
    cv.setFont("HN-B", 40); cv.drawString(18*mm, split+88*mm, "Wochenplaner")
    cv.setFont("HN-L", 17); cv.setFillColor(colors.HexColor("#ffffffcc"))
    cv.drawString(18*mm, split+75*mm, "Undatiert  ·  Fur jedes Jahr")
    ...
    cv.drawString(18*mm, split-16*mm, "Inhalt:")
    items = ["  ●  Jahresubersicht",
             "  ●  Monatsubersicht (undatiert)",
             "  ●  Wochenplaner (undatiert)",
             "  ●  Habit Tracker",
             "  ●  Notizseiten"]
    ...
    cv.drawString(23*mm, MB+21*mm,
        "Tipp: Drucke die Wochenplaner-Seite beliebig oft - fur jede Woche des Jahres.")
...
# ─── JAHRESÜBERSICHT ─────────────────────────────────────────
def jahres(cv, t):
    bg(cv, t); hdr(cv, t, "Jahresubersicht", "______")
    gx = 14*mm; gy = H-38*mm
    ...
    ftr(cv, t, "Jahresubersicht"); cv.showPage()

# ─── MONATSÜBERSICHT ─────────────────────────────────────────
def monats(cv, t):
    bg(cv, t); hdr(cv, t, "Monatsubersicht", "_____________ · ______")
    ...
    for wk in range(4):
        ...
        cv.drawCentredString(wx+wcw/2, yt-5*mm, f"Woche {wk+1}")
    ...
    cv.drawString(lx, ny-4*mm, "Notizen & Ziele des Monats")
    ...
    ftr(cv, t, "Monatsubersicht"); cv.showPage()

# ─── WOCHENPLANER ────────────────────────────────────────────
def woche(cv, t):
    bg(cv, t); hdr(cv, t, "Wochenplaner", "KW ____  ·  ____. _____________ ______")
    ...
    for i,(ab,full) in enumerate(DAYS):
        ...
        cv.drawCentredString(lx+dw-12.5*mm, yt2-5*mm, "Prioritat des Tages")
        # Write lines
        for l in range(lpd):
            ly2=yt2-11*mm-l*lg
            if ly2>yb2+2*mm:
    ...
    # Sidebar
    secs=[("Diese Woche",3),("Ziele der Woche",4),("Notizen",7),("Reflexion",3)]
    ...
    ftr(cv, t, "Wochenplaner Undatiert"); cv.showPage()
...
def habit(cv, t):
    ...
    rh=(yt-MB-20*mm)/(nh+1)
    habits=["Sport / Bewegung","Wasser (2 Liter)","Gesund ernahren",
            "Lesen (30 Min.)","Meditation","Fruh aufstehen",
            "Kein Social Media","Journaling","Dankbarkeit",
            "Eigenes Ziel: ___________"]
    # Day number header
    for d in range(nd):
    ...
    cv.drawString(lx+5*mm, qy-18*mm, "Mein Monatsziel:  ________________________________________________")
    ftr(cv, t, "Habit Tracker"); cv.showPage()

# ─── NOTIZEN ─────────────────────────────────────────────────
def notizen(cv, t):
    bg(cv, t); hdr(cv, t, "Notizen", "")
    lx=14*mm; rx=W-14*mm; y=H-43*mm
    while y>MB+10*mm:
        cv.setStrokeColor(t["R"]); cv.setLineWidth(0.4); cv.line(lx, y, rx, y); y-=9*mm
    ftr(cv, t, "Notizen"); cv.showPage()
...
for tname, tp in THEMES.items():
    out = f"{OUT}/wochenplaner_DE_{tname}.pdf"
